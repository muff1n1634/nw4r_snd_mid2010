                                          ...
                                          int voiceOutCount)
{
	...
	InitParam();
	mChannelCount = ut::Min(allocChannelCount, 16);
	...
	mTrackCount = ut::Min(trackIndex, 8);
	if (mTrackCount == 0)
...
void StrmPlayer::Shutdown()
{
	...
	NW4RAssert_Line(191, mStrmDataLoadTaskPool.Count() == BUFFER_BLOCK_COUNT_MAX);
...
{
	{
		...
		for (int trackIndex = 0; trackIndex < 8; trackIndex++)
		{
			...
			{
				Voice *voice = mTracks[trackIndex].mVoice;
				if (voice)
					voice->Stop();
			}
		}
	...
	NW4RAssert_Line(435, mStrmDataLoadTaskPool.Count() == BUFFER_BLOCK_COUNT_MAX);
}
                                ...
                                int bufferBlockIndex, bool needUpdateAdpcmLoop)
{
	...
	while (currentChannel < mChannelCount)
	{
		...
		int loadChannelCount = CHANNEL_MAX;
		...
		for (int i = 0; i < loadChannelCount; i++)
		{
			...
			u32 len = blockSize;
			void *source =
				ut::AddOffsetToPtr(sLoadBuffer, blockSize * i);
			void *dest = ut::AddOffsetToPtr(
				mChannels[currentChannel].bufferAddress, mDataBlockSize * bufferBlockIndex);
...
bool StrmPlayer::AllocVoices(int voiceOutCount)
{
	...
	for (int trackIndex = 0; trackIndex < mTrackCount; trackIndex++)
	{
		...
		Voice *voice = VoiceManager::GetInstance().AllocVoice(
			track.mTrackInfo.channelCount, voiceOutCount, 255,
...
void StrmPlayer::UpdateBuffer()
{
	...
	if (!mPlayFinishFlag && !mNoRealtimeLoadFlag && !mLoadWaitFlag)
	{
		...
		int axCurentBlockIndex = playingSample / mStrmInfo.blockSamples;

		while (mPlayingBufferBlockIndex != axCurentBlockIndex)
		{
			if (!mLoadWaitFlag && !mStrmDataLoadTaskList.IsEmpty()
...
{
#if 0
    long startBlockNum; // r27
    class AutoInterruptLock lock; // r31+0x8
    int trackIndex; // r26
    struct StrmTrack & track; // r29
    class Voice * voice; // r25
    int channelIndex; // r28
    struct StrmChannel * channel; // r23
#endif
...
{
	{
		...
		for (int trackIndex = 0; trackIndex < mTrackCount; trackIndex++)
		{
			     ...
			     channelIndex < track.mTrackInfo.channelCount; channelIndex++)
			{
				StrmChannel *channel = GetTrackChannel(track, channelIndex);
				voice->StopAtPoint(
					channelIndex, channel->bufferAddress,
					mStrmInfo.lastBlockSamples
						+ (endBufferBlockIndex * mStrmInfo.blockSamples));
			}
		}
	}
...
void StrmPlayer::UpdateLoadingBlockIndex()
{
	...
	bool needUpdateAdpcmLoop =
		mLoadingBufferBlockIndex == 0
		&& mStrmInfo.sampleFormat == SAMPLE_FORMAT_DSP_ADPCM;
...
// TODO: match without extra temps
bool StrmPlayer::CalcStartOffset(s32 *startBlockIndex, u32 *startBlockOffset,
                                 s32 *loopCount)
{
# if 0
	// Local variables
	int startOffsetSamples; // r29
	long loopStart;         // r28
	long loopEnd;           // r27
	long loopLen;           // r26
	long startOffset2;      // r25
# endif
...
#if 0
			startOffsetSamples =
				loopStart + (startOffset2 - startOffset2 / loopLen * loopLen);
#else
			s32 startLoop = startOffset2 / loopLen;
			startOffsetSamples =
				loopStart + (startOffset2 - (startLoop * loopLen));
#endif
		}
		else
		{
...
#if 0
	*startBlockOffset = startOffsetSamples
	                  - (startOffsetSamples / mStrmInfo.blockSamples
	                     * mStrmInfo.blockSamples);
#else
	s32 startBlock = startOffsetSamples / mStrmInfo.blockSamples;
	*startBlockOffset =
		startOffsetSamples - (startBlock * mStrmInfo.blockSamples);
#endif
	return true;
}
...
StrmChannel *StrmPlayer::GetTrackChannel(const StrmTrack &track, int channelIndex)
{
...
void StrmPlayer::StrmHeaderLoadTask::Execute()
{
	...
	if (!player->LoadHeader(fileStream, startOffsetType, startOffset))
...
{
	if (!mStrmPlayer->LoadStreamData(fileStream, mOffset, mSize, mBlockSize,
