...
{
	// .bss
	s16 volatile SeqPlayer::mGlobalVariable[GLOBAL_VARIABLE_NUM];
	...
	mSkipTimeCounter		(0.0f)
{
	...
	mParserParam.volume		= DEFAULT_VOLUME;
	mParserParam.priority	= DEFAULT_PRIORITY;
	mParserParam.callback	= nullptr;

	for (int varNo = 0; varNo < LOCAL_VARIABLE_NUM; varNo++)
		mLocalVariable[varNo] = DEFAULT_VARIABLE_VALUE;

	for (int trackNum = 0; trackNum < TRACK_NUM; trackNum++)
		mTracks[trackNum] = nullptr;
}
                                        ...
                                        NoteOnCallback *callback)
{
	...
	SeqPlayer::Stop(); // NOTE: qualified lookup to inhibit dynamic dispatch
...

	{
		...
		for (u32 trackBitMask = allocTracks; trackBitMask != 0;
		     trackBitMask >>= 1)
		{
			if (trackBitMask & 1)
				trackCount++;
		...
		for (int trackNo = 0; trackBitMask != 0; trackBitMask >>= 1, trackNo++)
		{
			if (trackBitMask & 1)
			{
...
// SeqPlayer::GetGlobalVariable? maybe all of them?
...
void SeqPlayer::CloseTrack(int trackNo)
{
	...
	// specifically not the source version
	NW4RAssertHeaderClampedLValue_Line(609, trackNo, 0, 16);
...
volatile s16 *SeqPlayer::GetVariablePtr(int varNo)
{
	// specifically not the source version
	NW4RAssertHeaderClampedLRValue_Line(746, varNo, 0, VARIABLE_NUM);
