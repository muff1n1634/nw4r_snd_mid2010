...
#include "nw4r/snd/Util.h" // GetRemoteFilterCoefs
...
void AxVoice::Setup(const void *waveAddr, SampleFormat format, int sampleRate)
{
...
bool AxVoice::IsPlayFinished() const
{
	...
	const void *zeroBuffer = AxManager::GetInstance().GetZeroBufferAddress();
	...
	default:
		NW4RAssertMessage_Line(103, false, "Invalid format\n");
		return false;
	}
...
void AxVoice::SetLoopStart(const void *baseAddress, u32 samples)
{
	ut::AutoInterruptLock lock;
...
void AxVoice::SetLoopEnd(const void *baseAddress, u32 samples)
{
	ut::AutoInterruptLock lock;
...
void AxVoice::StopAtPoint(const void *baseAddress, u32 samples)
{
	ut::AutoInterruptLock lock;
...
bool AxVoice::IsDataAddressCoverd(const void *beginAddress,
                                  const void *endAddress) const
{
	...
	// NOTE: parentheses necessary to match on release
...
u32 AxVoice::GetDspAddressBySample(const void *baseAddress, u32 samples,
                                    SampleFormat format)
{
	if (baseAddress)
...
u32 AxVoice::GetSampleByDspAddress(const void *baseAddress, u32 addr,
                                   SampleFormat format)
{
	if (baseAddress)
		baseAddress = OSCachedToPhysical(const_cast<void *>(baseAddress));
...
void AxVoice::SetAddr(bool loopFlag, const void *waveAddr, u32 startOffset,
...
bool AxVoice::IsNeedNextUpdate(const MixParam &param) const
{
...
void AxVoice::SetBiquad(u8 filterType, f32 value)
{
	...
	const BiquadFilterCallback *biquadFilter =
	...
	else
	{
		...
		{
			mVpb.SetVoiceBiquadCoefs(coef.b0, coef.b1, coef.b2, coef.a1, coef.a2);
		}
		else
		{
			AXPBBIQUAD biquad;
                                   ...
                                   const AdpcmParam &adpcmParam)
{
	...
	u32 currentPos = GetDspAddressBySample(dataAddr, 0, SAMPLE_FORMAT_DSP_ADPCM);
	u32 endPos = GetDspAddressBySample(dataAddr, offset, SAMPLE_FORMAT_DSP_ADPCM);
...
AxVoiceParamBlock::AxVoiceParamBlock():
	...
	mVolume				(AX_MAX_VOLUME)
{
	mPrevVeSetting.currentVolume	= AX_MAX_VOLUME;
...
void AxVoiceParamBlock::SetVoiceMix(const AXPBMIX &mix, bool immediatelySync)
{
	ut::AutoInterruptLock lock;
	...
	u16 *src = const_cast<u16 *>(reinterpret_cast<const u16 *>(&mix));
...
void AxVoiceParamBlock::SetVoiceAdpcmLoop(const AXPBADPCMLOOP &adpcmloop)
{
	ut::AutoInterruptLock lock;
...
void AxVoiceParamBlock::SetVoiceBiquad(const AXPBBIQUAD &biquad)
{
	ut::AutoInterruptLock lock;
...
void AxVoiceParamBlock::SetVoiceRmtIIR(const __AXPBRMTIIR &iir)
{
	ut::AutoInterruptLock lock;
