...
bool SeqFileReader::IsValidFileHeader(void const *seqData)
{
	...
	NW4RAssertMessage_Line(43, fileHeader->signature == SIGNATURE,
	            "invalid file signature. seq data is not available.");
	if (fileHeader->signature != SIGNATURE)
		return false;

	byte2_t version = Util::ReadBigEndian(fileHeader->version);

	NW4RAssertMessage_Line(
		51, version >= NW4R_VERSION(1, 0),
                                      ...
                                      u32 *offsetPtr) const
{
	...
	SeqFile::LabelBlock const *labelBlock =
		reinterpret_cast<SeqFile::LabelBlock const *>(ut::AddOffsetToPtr<u32>(
	     ...
	     index < Util::ReadBigEndian(labelBlock->labelInfoTable.count); ++index)
	{
		...
		// NOTE: reinterpret_cast also necessary here too
