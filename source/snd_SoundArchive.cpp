...
SoundType SoundArchive::GetSoundType(u32 soundId) const
{
                                                    ...
                                                    int bufferSize) const
{
	...
	if (fileInfo.extFilePath)
	{
		ut::FileStream *stream = OpenExtStreamImpl(
			buffer, bufferSize, fileInfo.extFilePath, 0, fileInfo.fileSize);
	...
	u32 offset = groupInfo.offset + itemInfo.offset;
	u32 size = itemInfo.size;

	if (groupInfo.extFilePath)
	{
		ut::FileStream *stream = OpenExtStreamImpl(
			buffer, bufferSize, groupInfo.extFilePath, offset, size);

		return stream;
	}
	else
	{
		ut::FileStream *stream = OpenStream(buffer, bufferSize, offset, size);
