...
{
	const ut::detail::RuntimeTypeInfo BasicSound::typeInfo(nullptr);
...
BasicSound::BasicSound(int priority, int ambientPriority):
	...
	at_0xdc				(0)
{
	NW4RAssertHeaderClampedLRValue_Line(53, priority, 0, 127);
...
void BasicSound::Update()
{
	...
	if (basicPlayer.IsStarted() && mUpdateCounter < -1ul)
		mUpdateCounter++;
...
{
	NW4RAssertHeaderClampedLRValue_Line(797, priority, 0, 127);
...
{
	NW4RAssertHeaderClampedLValue_Line(1000, bus, AUX_A, AUX_BUS_NUM);
