...
bool SoundArchivePlayer::IsAvailable() const
{
	 ...
	 * not only does ensure() not work here, !mSoundArchive doesn't work either
	 * ???
	 */
...
u32 SoundArchivePlayer::GetRequiredMemSize(SoundArchive const *arc)
{
	...
	size += ut::RoundUp(4 + arc->GetGroupCount() * 8, 4);
...
bool SoundArchivePlayer::CreateGroupAddressTable(SoundArchive const *arc,
                                                 void **buffer, void *endp)
{
	u32 requireSize = sizeof(u32) + arc->GetGroupCount() * sizeof(GroupAddress);
	...
	mGroupTable = static_cast<GroupAddress::Table *>(*buffer);
	*buffer = ep;
...
{
	u32 requireSize =
		sizeof(u32) + arc->detail_GetFileCount() * sizeof(FileAddress);
	...
	mFileTable = static_cast<FileAddress::Table *>(*buffer);
	*buffer = ep;
...
{
	u32 requireSize = numSounds * sizeof(detail::SeqSound);
...
bool SoundArchivePlayer::SetupWaveSound(SoundArchive const *arc ATTR_UNUSED,
                                        ...
                                        void *endp)
{
	u32 requireSize = numSounds * sizeof(detail::WaveSound);
...
bool SoundArchivePlayer::SetupStrmSound(SoundArchive const *arc ATTR_UNUSED,
                                        ...
                                        void *endp)
{
	u32 requireSize = numSounds * sizeof(detail::StrmSound);
...
{
	u32 requireSize = numTracks * sizeof(detail::MmlSeqTrack);
...
{
	// specifically not the source version
	NW4RAssertHeaderClampedLValue_Line(697, playerId, 0, mSoundPlayerCount);

	return mSoundPlayers[playerId];
}

// SoundArchivePlayer::GetSoundPlayer(const char *) ([R89JEL]:/bin/RVL/Debug/mainD.MAP:14216)
...
void const *SoundArchivePlayer::detail_GetFileWaveDataAddress(u32 fileId) const
{
	...
	for (unsigned i = 0; i < fileInfo.filePosCount; i++)
	{
		                                             ...
		                                             filePos.index, &itemInfo))
		{
			...
			return static_cast<byte_t const *>(groupData)
...
// SoundArchivePlayer::SetGroupAddress
...
// SoundArchivePlayer::SetFileAddress
	...
	void const *externalSeqDataAddress, char const *externalSeqStartLabel)
{
	...
	struct SeqFile {} const *seqFile = nullptr;
	...
	{
		seqFile = static_cast<SeqFile const *>(externalSeqDataAddress);
		seqOffset = 0;
	...
	if (!seqFile)
	{
		seqFile = static_cast<SeqFile const *>(
	...
	SoundStartable::StartInfo::StartOffsetType startOffsetType, int startOffset)
{
	...
	void *fileStreamBuffer = sound->GetFileStreamBuffer();
	u32 fileStreamBufferSize = sound->GetFileStreamBufferSize();
