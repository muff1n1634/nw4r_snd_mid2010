...
void SeqTrack::SetPlayerTrackNo(int playerTrackNo)
{
	...
	NW4RAssertHeaderClampedLRValue_Line(38, playerTrackNo, 0, 16);
...
void SeqTrack::InitParam()
{
	...
	mParserTrackParam.muteFlag			= 0;
	mParserTrackParam.silenceFlag		= 0;
	mParserTrackParam.noteFinishWait	= 0;
	mParserTrackParam.portaFlag			= 0;
	mParserTrackParam.damperFlag		= 0;
	mParserTrackParam.bankNo			= 0;
	mParserTrackParam.prgNo				= 0;
	mParserTrackParam.lfoParam.Init();
	mParserTrackParam.lfoTarget			= false;
	...
	mParserTrackParam.attack			= 255;
	mParserTrackParam.decay				= 255;
	mParserTrackParam.sustain			= 255;
	mParserTrackParam.release			= 255;
	mParserTrackParam.envHold			= 255;
	...
	for (int varNo = 0; varNo < VARIABLE_NUM; varNo++)
		mTrackVariable[varNo] = DEFAULT_VARIABLE_VALUE;
}
...
void SeqTrack::ReleaseAllChannel(int release)
{
	     ...
	     channel = channel->GetNextTrackChannel())
	{
		if (channel->IsActive())
		{
			if (release >= 0)
			{
				NW4RAssertHeaderClampedLRValue_Line(329, release, 0, 127);
...
{
	NW4RAssertHeaderClampedLRValue_Line(754, varNo, 0, VARIABLE_NUM);
