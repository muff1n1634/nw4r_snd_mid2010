...
#define DF_FUNCTION_WITH_PROTO(ident_)	\
 ...
 * Only works at global or namespace scope.
 *
 * Examples:
 * DECOMP_FORCE(UI2D_CONSTANT);
 * DECOMP_FORCE(1.0f, SI2D_CONSTANT);
 *
 * DECOMP_FORCE_EX()
 * {
...
#define DECOMP_FORCE(arg_)											\
	DF_FUNCTION_WITH_PROTO(DF_UNIQUE_IDENT(DECOMP_FORCE))			\
	{																\
...
#define DECOMP_FORCE_EX()											\
	DF_FUNCTION_WITH_PROTO(DF_UNIQUE_IDENT(DECOMP_FORCE_EX))

/*******************************************************************************
 * DECOMP_FORCE helpers
 */

// Conversion constants from int to float/double

extern   signed int DECOMP_SI;
extern unsigned int DECOMP_UI;

/* NOTE: If one or both of these constants appear at the beginning or in the
 * middle of a set of unused constants, you must end the current DECOMP_FORCE
 * macro with the constant(s) and start another DECOMP_FORCE macro to ensure
 * the constants are generated in the correct order.
 */
